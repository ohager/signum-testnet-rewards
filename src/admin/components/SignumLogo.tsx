 ...
 *
 * Inlined rather than loaded as an <img>: a single path costs less than the
 * request it replaces, and `currentColor` lets the mark follow the active
 * theme, which matters here because the admin panel ships five of them.
 *
 * The glyph is a KNOCKOUT: the path fills the disc and cuts the sigil out of
 * it, so the "ink" of the symbol is whatever is behind the logo.
 */
export function SignumLogo({ size = 32, className }: { size?: number; className?: string }) {
  return (
    <svg
