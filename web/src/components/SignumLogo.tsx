/*
 * The Signum logomark, from signum-node (GPL-3.0),
 * html/ui/assets/signum-logomark-white.svg. Reused here under the same licence.
 *
 * Inlined rather than loaded as an <img>: the artwork is a single path, so
 * inlining costs less than the request it replaces, and it can take its colour
 * from the theme. The source file hard-codes #fff; `currentColor` here lets the
 * mark follow whatever `color` its container sets, which is what keeps it
 * consistent with the rest of the page rather than a fixed white on dark blue.
 *
 * The glyph is a KNOCKOUT: the path fills the disc and cuts the sigil out of
 * it, so the "ink" of the symbol is whatever is behind the logo. It therefore
 * needs a background to sit on, and must not be placed on a busy one.
 */
export function SignumLogo({
  size = 32,
  className,
  title = "Signum",
}: {
